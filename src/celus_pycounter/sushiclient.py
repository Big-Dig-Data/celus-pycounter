...
from celus_pycounter import sushi
from celus_pycounter.helpers import convert_date_run, last_day, prev_month
...
def main(
    ...
    if status:
        ...
        sys.exit(0)
    click.echo(f"celus_pycounter SUSHI client for URL {url} ({report} R{release})")
